...
def step_upload_file(context, relative_path):
    ...
    upload_button = context.page.get_by_text("Upload image")
...
def step_see_preview(context):
    expect(
        context.page.get_by_text("Preprocessed (28×28)", exact=False)
...
def step_prediction_block(context):
    expect(
        context.page.get_by_text("Result", exact=False)
    ).to_be_visible(timeout=8000)

    expect(
        context.page.get_by_text("Predicted digit", exact=False)
    ).to_be_visible(timeout=8000)
...
def step_confidence(context):
    expect(
        context.page.get_by_text("Confidence", exact=False)
    ).to_be_visible(timeout=8000)
...
def step_top3(context):
    expect(
        context.page.get_by_text("Top-3", exact=False)
    ).to_be_visible(timeout=8000)
...
def step_error(context):
    expect(
        context.page.get_by_text("File is too large", exact=False)
    ).to_be_visible(timeout=8000)
